  ...
  const performClassification = async () => {
    ...
    try {
      ...
      if (result && 
        ...
        console.log(`🎯 Classified: ${result.label} (${(result.confidence * 100).toFixed(1)}%)`);
      }
    } catch (error) {
    ...
    const sketch = (p: p5) => {
      const gotPoses = (results: any) => {
        ...
        if (!results || results.length === 0) {
          const now = Date.now();
          if (currentPrediction && now - currentPrediction.timestamp > 3000) {
      ...
      const drawPredictionOverlay = () => {
        if (!currentPrediction || !posesRef.current || posesRef.current.length === 0) return;
        ...
        p.fill(0, 0, 0, 200);
        p.noStroke();
        p.rect(10, 10, 350, 120, 15);
        
        // Header
        p.fill(255);
        p.textSize(16);
        p.textAlign(p.LEFT, p.TOP);
        p.text('🤖 AI Detection:', 25, 30);
        
        // Pose name - large and prominent
        p.fill(r, g, b);
        p.textSize(28);
        p.textStyle(p.BOLD);
        p.text(currentPrediction.label.toUpperCase(), 25, 55);
        
        // Confidence
        p.fill(255);
        p.textSize(14);
        p.textStyle(p.NORMAL);
        p.text(`Confidence: ${(currentPrediction.confidence * 100).toFixed(1)}%`, 25, 90);
        
        // Confidence bar
        const barWidth = 300;
        const barHeight = 8;
        p.fill(255, 255, 255, 80);
        p.rect(25, 105, barWidth, barHeight, 4);
        p.fill(r, g, b, 220);
        p.rect(25, 105, barWidth * currentPrediction.confidence, barHeight, 4);
        ...
        p.text(`${timeSince.toFixed(1)}s ago`, 345, 90);
      };
      ...
      const drawPoseHighlight = () => {
        ...
        posesRef.current.forEach((pose) => {
          ...
          if (validKeypoints.length > 5) {
            ...
            p.fill(0, 0, 0, 180);
            p.noStroke();
            p.rect(centerX - 80, labelY - 12, 160, 28, 8);
    ...
    <div className="w-full space-y-6">
      ...
      {!isLoading && currentPrediction && (
        <div className={`${getCardStyles()} p-6`}>
          <h3 className={`${colors.primaryText} font-semibold text-lg mb-4`}>🎯 Current Prediction</h3>
